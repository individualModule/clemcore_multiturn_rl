...
class BatchRolloutBuffer(RolloutBuffer):
    ...
    def __init__(self, game_env: GameEnv):
        ...
        self.trajectories: List[List[dict]] = []  # Completed trajectories
    ...
    def on_step(self, env_id: int, context, response, done, info):
        ...
        if env_id not in self.active_trajectories:
            self.active_trajectories[env_id] = []  # Initialize a new trajectory for the environment
        self.active_trajectories[env_id].append(step)

    def on_done(self, env_id: int):
        """
        Finalize the trajectory for the given environment ID and move it to the completed trajectories list.
        """
    ...
    def drop_trajectory(self, env_id: int):
        ...
        """
        if env_id in self.active_trajectories:
            self.active_trajectories.pop(env_id)  # Remove the incomplete trajectory

    def reset_active_trajectories(self):
        """
        Reset the active trajectories, clearing all incomplete trajectories.
        """
        self.active_trajectories = {}

    def reset(self):
        """
        Reset the buffer, clearing all trajectories.
        """
        self.trajectories = []
        self.active_trajectories = {}
...
class BatchReplayBuffer(StepRolloutBuffer):
    """
    Replay buffer with a fixed size that removes the oldest samples when full.
    Stores incomplete trajectories in `active_trajectories` and moves them to `trajectories` only when completed.
    """

    def __init__(self, game_env: GameEnv, buffer_size: int, sample_size: int):
        """
        Args:
            game_env: The game environment instance.
    ...
    def flatten_steps(self):
        """
        Flatten trajectories into individual steps for sampling.
        """
        self.steps = []
    ...
    def load_buffer(self, file_path: str):
        """
        Load the buffer (trajectories and active trajectories) from a file.
        """
...
class ReplayBuffer(StepRolloutBuffer):
    ...
    def on_done(self):
        ...
        excess_count = len(self.trajectories) - self.buffer_size
        print(f"current_buffer_trajectories: {len(self.trajectories)}")
        if excess_count > 0:
            self.trajectories = self.trajectories[excess_count:]  # Keep only the most recent trajectories
