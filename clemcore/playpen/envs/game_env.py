...
class EvalBatchEnv(PlayPenEnv):
    ...
    def __init__(self, game: GameBenchmark, player_models: List[Model], task_iterator: GameInstanceIterator,
        ...
        Class is used for evaluation inside the training loop. 
        Iterates over All instances in the eval instances json. 
        Needs cleanup - create a common base class with reguar batch env and share common logic
        The base.py rollout classes need the same.
    ...
    def generate_envs(self):
        ...
        for i in range(self.len_active):
            envs[i] = ExecuteSingle(self._game, self._player_models, self.queue.pop(0))
        return envs
    ...
    def on_done(self, env_id: int):
        if env_id in self.envs:
            ...
            raise RuntimeError(f"No environment found: '{env_id}'")
    ...
    def get_rollout_length(self):
        return self.initial_queue_size
...
class ExecuteSingle(PlayPenEnv):
    """
    Game env without task iterator -> executes single environment it was assigned. Takes single instance input
    """
    def __init__(self, game: GameBenchmark, player_models: List[Model], single_instance: Tuple,
                 reset=True):
        super().__init__()
        self._game = game
        self._game_name = game.game_name
    ...
    def reset(self) -> None:
        self._experiment, self._game_instance = self.single_instance
        self.master = self._game.create_game_master(self._experiment, self._player_models)
        self.master.game_recorder = DefaultGameRecorder(self._game_name,
                                                        self._experiment["name"],
                                                        self._game_instance["game_id"],
